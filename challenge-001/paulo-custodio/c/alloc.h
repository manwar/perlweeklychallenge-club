...
static void intarray_reverse(IntArray* arr) {
    for (int i = 0; i < arr->size/2; i++) {
        SWAP(int, arr->data[i], arr->data[arr->size - 1 - i]);
    }
}
...
static void doublearray_reverse(DoubleArray* arr) {
    for (int i = 0; i < arr->size/2; i++) {
        SWAP(double, arr->data[i], arr->data[arr->size - 1 - i]);
    }
}
...
static void str_sort(Str* str) {
    qsort(str->body, str->size, sizeof(char), char_compare);
}

static void str_reverse(Str* str) {
    for (int i = 0; i < str->size/2; i++) {
        SWAP(char, str->body[i], str->body[str->size - 1 - i]);
    }
}
...
static void str_uniq(Str* str) {
    str_sort(str);
    int w = 0;
    for (int r = 0; r < str->size; r++) {
        if (w > 0 && str->body[w-1] == str->body[r])
            continue;
...
static void strarray_reverse(StrArray* arr) {
    for (int i = 0; i < arr->size/2; i++) {
        SWAP(char*, arr->data[i], arr->data[arr->size - 1 - i]);
    }
}
