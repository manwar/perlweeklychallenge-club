...
set range [expr $last_ord - $first_ord]
set in_size 5

append pi {%(uP7+Y`ME)gy(7')v?;%s^qc(Mj1"16idp'ONGs'2m.o,1HLY'"~P61@@Y-} \
...
puts -nonewline "3."
for {set i 0} {$i < [string length $pi]} {incr i $in_size} {
    set out 0
    for {set j $i} {$j < [expr $i + $in_size]} {incr j} {
        scan [string index $pi $j] %c ch
        if {$ch > $skip_ord} {incr ch -1}
        set out [expr $out * $range + $ch - $first_ord]
    }
    puts -nonewline [format "%09d" $out]
}
