...
let in_size   = 5
let out_size  = 9
let first_ord = "#"  . codePointAt (0)
let skip_ord  = "\\" . codePointAt (0)
let last_ord  = "~"  . codePointAt (0)
let range     = last_ord - first_ord
let format    = printf ("%%0%dd", out_size)

process . stdout . write ("3.")

for (let i = 0; i < pi . length; i += in_size) {
    let out = 0
    for (let j = i; j < i + in_size; j ++) {
        out *= range
        let n = pi . codePointAt (j)
        if (n >= skip_ord) {
            n --
        }
        out += n - first_ord
    }
    printf (process . stdout, format, out)
}
