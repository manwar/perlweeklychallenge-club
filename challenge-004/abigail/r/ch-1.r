...
first_ord <- as.integer (charToRaw ('"')) + 1
last_ord  <- as.integer (charToRaw ('~'))
skip_ord  <- as.integer (charToRaw ('\\')) - first_ord
range     <- last_ord - first_ord
in_size   <- 5

pi <- as.integer (charToRaw (paste (
