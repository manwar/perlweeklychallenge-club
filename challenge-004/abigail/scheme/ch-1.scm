...
(define first_ord (char->integer #\#))
(define last_ord  (char->integer #\~))
(define skip_ord  (char->integer #\\))
(define range     (- last_ord first_ord))
(define in_size    5)

(define pi (map-in-order char->integer (string->list (string-append
...
(define (decode out chars)
    (if (= (length chars) 0) (format #t "~9,'0d" out)
        (decode (+ (* out range)
                   (- first_ord)
                   (car chars)
                   (if (> (car chars) skip_ord) -1 0))
                (cdr chars))))
...
(define (topi pi)
    (if (> (length pi) 0)
        (begin
            (decode 0 (list-head pi in_size))
            (topi     (list-tail pi in_size))
        )
    )
)
