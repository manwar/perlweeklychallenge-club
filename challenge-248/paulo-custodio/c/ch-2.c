...
IntMatrix* compute_sums(IntMatrix* m) {
    IntMatrix* sums = intmatrix_new(m->rows-1, m->cols-1);
    for (int r = 0; r < sums->rows; r++) {
        for (int c = 0; c < sums->cols; c++) {
            sums->data[r][c] = m->data[r][c] + m->data[r][c+1]
                             + m->data[r+1][c] + m->data[r+1][c+1];
        }
    }
    return sums;
}
...
int main(int argc, char* argv[]) {
    ...
    IntMatrix* sums = compute_sums(m);
    intmatrix_print(sums);
