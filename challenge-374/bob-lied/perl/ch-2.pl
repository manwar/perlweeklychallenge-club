...
sub lsdn2($str)
{
    ...
    {
        my @span = before { $_ != $digit } @n;
        my $val = 0+join("", $digit, @span);

        $biggest = $val if $val > $biggest;

        splice(@n, 0, scalar(@span));
    }
    return $biggest;
}
...
sub runTest
{
    ...
    is( lsdn3("6777133339"),  3333,  "Example 1");
    is( lsdn3("1200034"   ),  4,     "Example 2");
    is( lsdn3("44221155"  ),  55,    "Example 3");
    is( lsdn3("88888"     ),  88888, "Example 4");
    is( lsdn3("11122233"  ),  222,   "Example 5");
