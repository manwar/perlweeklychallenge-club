...
./ch-1.pl "([1,-4,2,-2,-4,-5,7,-3],[17,3,76,3,7,3])"
...
sub sum_unique ($aref) {
   ...
   for ( @$aref ) {++$ab{$_}}                   # Record how many copies of each element exist.
   my $sum = 0;                                 # Variable to hold sum of unique elements
   map { $sum += $_ if 1 == $ab{$_} } keys %ab; # Sum of 1-of-a-kind elements.
...
for my $aref (@arrays) {
   say '';
   my $sum = sum_unique($aref);
   say "Array = (@$aref)";
   say "Sum of unique elements = $sum";
}
