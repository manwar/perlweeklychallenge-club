...
set cases {
    {{{{1 2 3}
       {4 5 6}
       {7 8 9}
      } 1} {{9 1 2} {3 4 5} {6 7 8}} "Example 1"}
    {{{{10 20}
       {30 40}
      } 1} {{40 10} {20 30}} "Example 2"}
    {{{{1 2}
       {3 4}
       {5 6}
      } 1} {{6 1} {2 3} {4 5}} "Example 3"}
    {{{{1 2 3}
       {4 5 6}
      } 5} {{2 3 4} {5 6 1}} "Example 4"}
    {{{{1 2 3 4}
      } 1} {{4 1 2 3}} "Example 5"}
}
...
proc shift_grid {p} {
    ...
    for {set i 0} {$i < $k} {incr i} {
        set carryover ""
        for {set idx 0} {$idx < [llength $grid]} {incr idx} {
            set row [lindex $grid $idx]
            if {$carryover != ""} {
                set row [linsert $row 0 $carryover]
            }
            set carryover [lindex $row end]
            set row [lreplace $row end end]
            set grid [lreplace $grid $idx $idx $row]
        }
        set row [lindex $grid 0]
        set row [linsert $row 0 $carryover]
        set grid [lreplace $grid 0 0 $row]
    }
    return $grid
}
