...
This handles the last column: elements in the last column of row i wrap to the f
irst column of the next row (i+1).
...
We take elements from the original grid at (i, j) and put them into new_grid[i][
j+1].
...
Elements from original last column (except last row) go to next row's first colu
mn.
...
[4, 1, 2, 3]
   ...
   Last date to submit the solution 23:59 (UK Time) Sunday 4th January
   2026.
     __________________________________________________________________
...
function TestSg()
	local td = {{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}, 1, {{9, 1, 2},{3, 4, 5},{6, 7, 8}},
	...
	local n = 3
	for i=1, #td, 3 do
		local src, k, dst = td[i], td[i+1], td[i+2]
		lu.assertEquals(sg(src, k), dst)
	end
end
