...
############################################################
#
# The rules are a complicated way to bascially say "move every
# element in the matrix to the right, just handing over the overflow
# into the next line unless it's the last line in which case the
# overflow goes into the first line". So this whole operation
# turns into something pretty straight forward: k times, we just
# do this simple operation of moving the last element of the last
# array in the matrix to the beginning of the first one, and then
# for all array starting in the first and ending in the
# second to last one, we just move the last element to the beginning
# of the next array.

use v5.36;

shift_grid([[1, 2, 3], [4, 5, 6], [7, 8, 9]], 1);
shift_grid([[10, 20], [30, 40]], 1);
shift_grid([[1, 2], [3, 4], [5, 6]], 1);
shift_grid([[1, 2, 3], [4, 5, 6]], 5);
...
sub shift_grid($matrix, $k) {
    ...
    say "       ], \$k = $k";
    foreach my $count (1..$k) {
        my $last = scalar(@$matrix) - 1;
        my $elem = pop(@{$matrix->[$last]});
        unshift @{$matrix->[0]}, $elem;
        foreach my $line (0..$last-1) {
            $elem = pop(@{$matrix->[$line]});
            unshift @{$matrix->[$line+1]}, $elem;
        }
    }
    say "Output: [";
    foreach my $array (@$matrix) {
