...
2. The shift count ($k) is entered on the command-line, followed by the input
   ...
   whitespace. For example, with $k = 3, the matrix:
    ...
    is entered as: >perl ch-2.pl 3 "1 2 3" "4 5 6" "7 8 9"
...
Usage:
  ...
  perl $0
    ...
    <k>               Shift count
    [<matrix> ...]    Non-empty integer matrix
END
...
{
    $| = 1;
    print "\nChallenge 354, Task #2: Shift Grid (Perl)\n\n";
}
...
#===============================================================================
{
    ...
    else
    {
        ...
        my $shifted = shift_matrix( $matrix, $k );

        print_matrix( 'Output: ', $shifted );
    }
}
...
#-------------------------------------------------------------------------------
{
    ...
    for (1 .. $k)
    {
        ...
        for my $i (0 .. $#shifted)
        {
            push @elements, @{ $shifted[$i] };
        }

        unshift @elements, pop @elements;
        ...
        {
            my   @row;
            push @row, shift @elements for 1 .. $cols;
            push @shifted,  \@row;
        }

        scalar @elements == 0 or die 'Impossible case';
    }
...
#-------------------------------------------------------------------------------
{
    ...
    for my $row (@$matrix)
    {
        ...
        if ($first)
        {
            printf "[%s]", join ', ', @row_str;
            $first = 0;
        }
        else
        {
            printf "\n%s [%s]", $tab, join ', ', @row_str;
        }
    }
...
#-------------------------------------------------------------------------------
{
    ...
    while (my $line = <DATA>)
    {
        ...
        my  ($test_name, $matrix_str, $k, $exp_str) = split / \| /x, $line;

        for ($test_name, $matrix_str, $k, $exp_str)
        {
            s/ ^ \s+   //x;
            s/   \s+ $ //x;
        }

        my @mat_rows = split  / ; /x, $matrix_str;
        my $matrix   = parse_matrix( \@mat_rows );
        my $shifted  = shift_matrix(  $matrix, $k );
        my @exp_rows = split  / ; /x, $exp_str;
        my $expected = parse_matrix( \@exp_rows );

        is_deeply $shifted, $expected, $test_name;
    }
