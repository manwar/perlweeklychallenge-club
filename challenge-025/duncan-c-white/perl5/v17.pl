...
fun findall()
{
	...
	for(;;)
	{
		...
		foreach my $path (@$currpaths)	# foreach current path
		{
			...
			foreach my $wno (grep { $avail->[$_] } @$inwords)
			{
				# alter the avail array, marking $wno used.
				$avail->[$wno] = 0;

				# build a whole new path, with length N+1
				# sequence, viz: cons(wno,oldseq)
				push @$newpaths,
					[
					  $inword[$wno], "$wno,$s", [ @$avail ]
					];
