...
def vtc(string: str) -> int:
    return sum(1 for e in re.finditer(r"\S+", string) if pattern.fullmatch(e.group(0)))

import unittest

class TestVtc(unittest.TestCase):
    def test(self):
        for inpt, otpt in {
                "cat and dog" : 3 ,
                "a-b c! d,e" : 2 ,
            ...
            self.assertEqual(vtc(inpt), otpt)
